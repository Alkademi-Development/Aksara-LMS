<template>
  <div class="container-fluid py-4">
    <FilterForms 
      ...
      :valueOfSelectedSort="valueOfSelectedSort"
      :valueOfSearchQuery="searchQuery"
    ...
    <section id="section-course">
      <div v-if="!isBusy">
        <div v-if="materi?.length > 0 && materi != null" class="row" style="gap: 24px 0px">
          ...
          <template v-for="index in 4">
            <div :key="`skeleton-loadmore-${index}`" v-if="isMateriLoadMore" class="col-12 col-md-6 col-lg-4">
      ...
      <div v-else class="row">
        <div v-for="index in 4" :key="`skeleton-card-${index}`" class="col-12 col-md-6 col-lg-3">
          <div style="height: 330px;">
            <b-skeleton-img height="330px"></b-skeleton-img>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async fetchCourses(){
      ...
      if(search) this.searchQuery = search, localStorage.removeItem("searchQuery");
      if(sort) this.selectedSort = sort, localStorage.removeItem("selectedSort");
      this.valueOfSelectedSort = this.sortMateri.find((item) => item?.key === this.selectedSort);
      ...
      await this.$store.dispatch('Services/getListModulesDraft', params)
    ...
    onClickMateri() {
        this.$router.push(`/dashboard/courses/materi/create`);
    },
    toggleDelete(data) {
