<template>
    <div class="custom-search-input">
        ...
        <b-card
            ...
            <template v-if="!isEmpty">
                <template v-if="options?.length != 0">
                    <!-- Show select all only on school selections -->
                    <div class="checkbox-container" v-if="id === 'checkbox-schools'">
                        <label :for="`${ id }-select-all`" class="checkbox-label" @click="stopPropagation($event); toggleSelectAll">Pilih Semua</label>
                        <input :id="`${ id }-select-all`" type="checkbox" :checked="isAllSelected" @click="toggleSelectAll">
                    </div>
        ...
        <div id="city-selected" class="d-flex flex-wrap mx--1 mt-2 list-scroll">
...
export default {
    ...
    methods: {
        // Allow label select all to be able to trigger the select
