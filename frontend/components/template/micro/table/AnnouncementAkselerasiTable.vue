<template>
    <section id="student-table" class="card border">
        <b-table
            id="datatables"
            class="pb-4 m-0"
            head-variant="light"
        ...
        >
            ...
            <template #cell(type)="row">
                <span style="text-transform: capitalize">{{ !!row.item.type ? row.item.type : '-'}}</span>
            </template>
...
export default {
    ...
    methods: {
        ...
        onUpdatePage(val) {
            this.$emit('fetchData', val, 3)
        },
        onClickAction(item, type) {
            // if (type == 'preview') this.$router.push(`/dashboard/${this.res}/${item.id}`);
            if (type == 'create') this.$router.push(`announcement/create`)
            if (type == 'edit') {
                localStorage.setItem('announceId', toEncrypt(item?.id))
                this.$router.push({
                    path: `announcement/edit/${item.title.toLowerCase().replace(" ", "-")}`,
                })
            }
        ...
        async fetchDeleteItem(id) {
            ...
            .then(() => {
                ...
                if(!servicesState.status) {
                    console.error('Failed delete announcement', servicesState.message)
                    this.$emit('isLoading', true)
                } else {
