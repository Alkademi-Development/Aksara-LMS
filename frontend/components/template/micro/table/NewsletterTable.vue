      ...
      <div class="card border">
          <b-table
              ...
              <template #cell(content)="row">
                {{ row.item.content | stripHtmlTags | truncate }}
              </template>

              <template #cell(action)="row">

                  <b-button size="sm" variant="primary" @click="onClickShow(row.item)">
                      <span class="btn-inner--text">Lihat Newsletter</span>
                  </b-button>
...
export default {
  ...
  filters: {
    ...
    stripHtmlTags(value) {
      if (!value) return ''
      return value.replace(/<\/?[^>]+(>|$)/g, "") // Regex untuk hapus semua tag HTML
    },
  },
  mounted() {
  ...
  methods: {
      onClickCreate() {
          this.$router.push('/dashboard/newsletter/create')
      },
      onClickShow (item) {
        localStorage.setItem('newsletterId', toEncrypt(item?.id))
        this.$router.push({
            path: `/dashboard/newsletter/${item?.slug}`,
        });
      },
      onClickEdit (item) {
          localStorage.setItem('newsletterId', toEncrypt(item?.id))
          this.$router.push({
              path: `/dashboard/newsletter/edit/${item?.slug}`,
          });
      },
      onClickDelete(item) {
