          ...
          <div v-if="isLoaded" class="col-lg-8 card-wrapper" >
              ...
              <b-card no-body>
                  <b-card-header class="border-0 pb-0">
                      <h5 class="mb-0 d-inline-block">Overview</h5>
                  </b-card-header>
                  <div class="card-body">
                      <b-alert 
              ...
              <b-card no-body v-if="isLoaded">
                  ...
                  <div class="card-body">
                      <b-row>
                          <b-col 
                              v-for="(input, index) in inputOthers" 
                              :key="index" 
                              cols="12"
                          >
                              <b-form-group
          ...
          <div v-if="isLoaded" class="col-lg-4 card-wrapper">
              <div class="row">
                  <div class="col-12">
                      <b-card no-body v-if="isLoaded">
                          <b-card-header class="border-0 pb-0">
                              <h5 class="my-1 d-inline-block">Detail Kelas</h5>
                          </b-card-header>
                          <div class="card-body">

                              <b-row>

                                  <template v-for="(input, index) in inputDetail">
                                      ...
                                      <b-col :key="index" cols="12">
                                          ...
                                          <b-form-group
                                              ...
                                              <SearchDropdownCheckbox 
                                                  ...
                                                  :placeholder="input?.placeholder"
                                                  :items="userOptions"
                                                  @onChange="usersSelected = $event"/>

                                          </b-form-group>

                                          <b-form-group
                                              v-if="input.name == 'mode'"
                                              :label="input.label"
                                              :label-for="input.label"
                                              :description="input.description"
                                          >
                                          ...
                                          <!-- <b-form-group
                                              v-if="input.name == 'level' && isLevelingCase"
                                              :label="input.label"
                                              :label-for="input.label"
                                              :description="input.description"
                                          >
                                          ...
                                          <!-- V Select for Select School -->
                                          <b-form-group
                                              v-if="input.name == 'schoolId' && isSchoolCase"
                                              :label="input.label"
                                              :label-for="input.label"
                                          >
                                              <v-select
                                          ...
                                          <b-form-group
                                              ...
                                              <SearchDropdownCheckbox 
                                                  ...
                                                  :placeholder="input?.placeholder"
                                                  :items="moduleOptions"
                                                  @onChange="modulesSelected = $event"/>
                  ...
                  <div class="col-12">
                      <b-card no-body v-if="isLoaded">
                          <b-card-header class="border-0 pb-0">
                              <h5 class="my-1 d-inline-block">Jadwal</h5>
                          </b-card-header>
                          <div class="card-body">

                              <b-row>
                                  <template v-for="(input, index) in inputDate">
                                      <b-col :key="index" cols="12" v-if="input?.name != 'certificateRelease'">
                                          <b-form-group
                                              :label="input.label"
                                              :label-for="input.label"
                                              :description="input.description"
                  ...
                  <div class="col-12" hidden>
                      <b-card no-body v-if="isLoaded">
                          <b-card-header class="border-0 pb-0">
                              <h3 class="my-1 d-inline-block">Pengaturan Lainnya</h3>
                          </b-card-header>
                          <div class="card-body">

                              <b-row>
                                  <template v-for="(input, index) in inputForm">
                                  <b-col :key="index" v-if="input.name == 'registrantCity' || input.name == 'cityHeld' || input.name == 'modules' || input.name == 'requirementFields'" cols="12" md="6" lg="12">
                                      <b-form-group
                                          :label="input.label"
                                          :label-for="input.label"
                                          :description="input.description"
                                      >   
                                          <b-button v-if="input.name == 'registrantCity'" :variant="citySelected.length == 0 ? 'light' : 'primary'" block @click="toggleModalSelect(input.name)">{{citySelected.length == 0 ? 'Pilih Kota' : `${citySelected.length} Dipilih`}}</b-button>
                                          <b-button v-if="input.name == 'cityHeld'" :variant="cityHeldSelected.length == 0 ? 'light' : 'primary'" block @click="toggleModalSelect(input.name)">{{cityHeldSelected.length == 0 ? 'Pilih Kota' : `${cityHeldSelected.length} Dipilih`}}</b-button>
                                          <b-button v-if="input.name == 'modules'" :variant="modulesSelected.length == 0 ? 'light' : 'primary'" block @click="toggleModalSelect(input.name)">{{modulesSelected.length == 0 ? 'Pilih Materi' : `${modulesSelected.length} Dipilih`}}</b-button>
                                          <b-button v-if="input.name == 'requirementFields'" :variant="requirementSelected.length == 0 ? 'light' : 'primary'" block @click="toggleModalSelect(input.name)">{{requirementSelected.length == 0 ? 'Pilih Kebutuhan Tambahan' : `${requirementSelected.length} Dipilih`}}</b-button>
                                      </b-form-group>

                                      <b-modal v-if="input.name == modalToShow" v-model="isModalSelectForm" size="lg" :title="input.label" hide-footer centered>

                                          <SelectDropdown 
                                              v-if="modalToShow == 'registrantCity'"
                                              selectedLabel="Kota Terpilih"
                                              :options="cityOptions" 
                                              :itemSelected="citySelected" 
                                              :placeholder="input.placeholder" 
                                              :isSelectedAll="isSelectedAllCities"
                                              @onChangeSelectedAll="isSelectedAllCities = $event"
                                              @onChangeItemSelected="citySelected = $event"/>

                                          <SelectDropdown 
                                              v-if="modalToShow == 'cityHeld'"
                                              selectedLabel="Kota Terpilih"
                                              :options="cityOptions" 
                                              :itemSelected="cityHeldSelected" 
                                              :placeholder="input.placeholder" 
                                              :isSelectedAll="isSelectedAllHeldCities"
                                              @onChangeSelectedAll="isSelectedAllHeldCities = $event"
                                              @onChangeItemSelected="cityHeldSelected = $event"/>

                                          <SelectDropdown 
                                              v-if="modalToShow == 'modules'"
                                              selectedLabel="Materi Terpilih"
                                              :options="moduleOptions"
                                              :itemSelected="modulesSelected"
                                              :placeholder="input.placeholder"
                                              @onChangeItemSelected="modulesSelected = $event"/>

                                          <SelectDropdown 
                                              v-if="modalToShow == 'requirementFields'"
                                              selectedLabel="Kebutuhan Terpilih"
                                              :options="requirementOptions"
                                              :itemSelected="requirementSelected"
                                              :placeholder="input.placeholder"
                                              @onChangeItemSelected="requirementSelected = $event"/>
...
export default {
  ...
  methods: {
      ...
      getCredentials() {
          ...
          this.inputForm = []
          this.inputOverview = fieldForm?.inputFormOverview
          this.inputDetail = fieldForm?.inputFormDetail
          this.inputDate = fieldForm?.inputFormDate
          this.inputOthers = fieldForm?.inputFormOthers
      ...
      async fetchClassroomDetail(){
          ...
          if(!this.servicesState.status) {
              ...
              this.inputOverview?.map((item, index) => {
                  if(item.name == 'logo') {
                      this.inputOverview[index].value = classroomDetail?.logo ?? null
                  }
                  if(item.name == 'name') {
                      this.inputOverview[index].value = classroomDetail?.name
                  }
                  if(item.name == 'description') {
                      this.inputOverview[index].value = classroomDetail?.description
                  }
                  if(item.name == 'minScore') {
                      this.inputOverview[index].value = classroomDetail?.minScore
                  }
                  if(item.name == 'tags') {
                      this.inputOverview[index].value = classroomDetail?.tags
                  }
                  if(item.name == 'quota') {
              ...
              this.inputDate?.map((item, index) => {
                  if (classroomDetail?.alwaysOpen) this.isAlwaysOpen = true
                      ...
                      if(item.name == 'registrationStart') {
                          this.inputDate[index].value = moment(classroomDetail?.registrationStart).format('YYYY-MM-DD HH:mm:ss')
                      }
                      if(item.name == 'registrationEnd') {
                          this.inputDate[index].value = moment(classroomDetail?.registrationEnd).format('YYYY-MM-DD HH:mm:ss')
                      }
                      if(item.name == 'start') {
                          this.inputDate[index].value = moment(classroomDetail?.start).format('YYYY-MM-DD HH:mm:ss')
                      }
                      if(item.name == 'end') {
                          this.inputDate[index].value = moment(classroomDetail?.end).format('YYYY-MM-DD HH:mm:ss')
                      }
                  }
              ...
              if(classroomDetail.registrarCity != null && classroomDetail.registrarCity != []) {
                  classroomDetail.registrarCity.map((item) => {
                      ...
                      if (cityIndex != -1) this.cityOptions[cityIndex].checked = true
                  })

                  this.cityOptions?.forEach((city) => {
                      if (city.checked) this.citySelected.push(city)
                  })
              }
              ...
              if(classroomDetail.heldCity != null && classroomDetail.heldCity != []) {
                  ...
                  this.cityHeldOptions?.forEach((city) => {
                      if (city.checked) this.cityHeldSelected.push(city)
                  })
              }
              ...
              if(classroomDetail.requirementFields != null && classroomDetail.requirementFields != []) {
                  ...
                  this.requirementOptions?.forEach((req) => {
                      if (req.checked) this.requirementSelected.push(req)
                  })
              }
      ...
      onSubmitForm(event) {
          ...
          if (!this.isSaveToDraft && !this.isEditPage) {
              ...
              if (!this.isUnlimited) {
                  ...
                  this.inputOverview?.forEach((item, index) => {
      ...
      processFormData(state) {
          ...
          this.inputForm = this.inputOverview.concat(this.inputOthers, this.inputDetail, this.inputDate)
          ...
          this.inputForm.map((item) => {
              if(item.type == 'datetime-local' && item.name != 'certificateRelease') {
                  if (this.isAlwaysOpen) {
                      this.output["alwaysOpen"] = this.isAlwaysOpen
                      this.output[item.name] = null
                  } else this.output[item.name] = moment(item.value).format('YYYY-MM-DDTHH:mm:ssZ')
              } else if(item.type == 'datetime-local' && item.name == 'certificateRelease') {
                      if (this.isPublishCertificate) this.output[item.name] = moment(this.publishCertificateDate).format('YYYY-MM-DDTHH:mm:ssZ')
                      else this.output[item.name] = null
                  ...
                  if (this.selectedSchool?.length > 0) {
                      school_id = []
                      this.selectedSchool?.map((item) => school_id.push(item.id))
                  }

                  if (this.schoolsSelected?.length > 0) {
                      school_id = []
                      this.schoolsSelected?.map((item) => school_id.push(item.value))
                  }
      ...
      async fetchListClass(){
          ...
          if(!this.servicesState?.status){
              ...
              if(this.action === 'edit') this.isLoaded = true;
          }
      }
  },
